...
import { type IUpdateProp } from '@/Interfaces/IHasUpdate';
...
export default class TravelScene implements IScene, IHasMenu, IHasUpdate {
  ...
  public constructor() {
    ...
    this._provider.name = 'Maquete';
    this._loader = new GLTFLoader();

    this._droneCamera = new Camera();
    this._droneControls = new OrbitControls(
      this._droneCamera.getProvider(),
      document.body,
    );
    ...
    this._personControls = new FlyControls(
      this._personCamera.getProvider(),
      document.body,
    );
    ...
    this._loader.load(KingModel, (_) => {
      ...
      if (max && min) {
        ...
        this._provider.add(new AxesHelper(1000));
      }
    });
  }
  ...
  onUnload(): TravelScene {
    this._menu?.getProvider().hide();
    return this;
  }

  onLoad(aLoader: unknown): TravelScene {
