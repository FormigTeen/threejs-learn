...
export default class BezierScene implements IScene, IHasMenu, IHasUpdate {
  ...
  onMenu(aMenu: IHasProvider<GUI>): IHasProvider<GUI> {
    if (!this._menu) {
      ...
      this._menu
        .getProvider()
        .add(this._controls, 'Velocidade', 0.01, 0.1)
  ...
  onUnload(): BezierScene {
    this._menu?.getProvider().hide();
    return this;
  }

  onLoad(aLoader: unknown): BezierScene {
  ...
  getVectors(): Vector3[] {
    return [
      new Vector3(
        this._controls.Escala * -1.0,
        (this._controls.Amplitude * Math.sin(0)) / 2.0,
        0.0,
      ),
      new Vector3(
        this._controls.Escala * -0.5,
        (this._controls.Amplitude * Math.sin(Math.PI / 2.0 + this._teta)) / 2.0,
        0.0,
      ),
      new Vector3(
        this._controls.Escala * 0.5,
        (this._controls.Amplitude *
          Math.sin((3.0 * Math.PI) / 2.0 + this._teta)) /
          2.0,
        0.0,
      ),
      new Vector3(
        this._controls.Escala * 1.0,
        (this._controls.Amplitude * Math.sin(Math.PI)) / 2.0,
        0.0,
      ),
    ];
  }
