...
export default class Table implements IHasProvider<Mesh>, IHasUpdate, IHasMenu {
  ...
  protected getGeometry(): PlaneGeometry {
    return new PlaneGeometry(100, 100, 10, 10);
  }
  ...
  protected getMaterial(): ShaderMaterial {
    ...
    const uniforms: IUniforms = {
      ...
      aCenters: {
        value: [
          ...centers,
          ...[...Array(LIMIT_POW * LIMIT_POW - centers.length).keys()].map(
            () => ({ aVertex: new Vector2(), isActive: false }),
          ),
        ],
      },
    };
  ...
  onMenu(aMenu: Menu): Menu {
    aMenu
      .getProvider()
      .add(this._controls, 'Raio', 0.1, 5.0)
      .onChange(() => this.onChange());
    aMenu
      .getProvider()
      .add(this._controls, 'Modo', Object.keys(this._modes))
      .onChange(() => this.onChange());
    aMenu
      .getProvider()
      .add(this._controls, 'Modo Tabuleiro')
      .onChange(() => this.onChange());
    aMenu
      .getProvider()
      .add(
        this._controls,
        'Potencia',
        [...Array(LIMIT_POW).keys()].map((_) => _ + 1),
      )
      .onChange(() => this.onChange());
    return aMenu;
  }
  ...
  public getCenters(): IVertexConfig[] {
    ...
    const getConfigVertex = (x: number, y: number): IVertexConfig => {
      const getOffsetPair = () =>
        this._controls.Potencia % 2 == 0 ? this.getRaio() : 0;
      const isActive = (ownX: number, ownY: number) => {
        if (!this._controls['Modo Tabuleiro']) return true;
    ...
    for (
      let aPhaseX = 0;
      aPhaseX < Math.floor(this._controls.Potencia / 2);
      aPhaseX++
    ) {
      if (modePair == 1) {
      ...
      for (
        let aPhaseY = 0;
        aPhaseY < Math.floor(this._controls.Potencia / 2);
        aPhaseY++
      ) {
